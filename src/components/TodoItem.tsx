...
function TodoItem({ id, text, completed, filtered, toggle, remove }: ComponentProps): JSX.Element {
    ...
    return (
        <li key={id} className={`${filtered === true ? 'hidden' : 'flex items-center break-all'} min-h-[52px] py-4 px-5 bg-white dark:bg-dark-800`}  >
            <button onClick={() => toggle(id)} className={`flex justify-center items-center w-5 h-5 lg:w-6 lg:h-6 rounded-full border shrink-0 ${completed ? 'bg-gradient-to-r from-gradient-1 to-gradient-2 border-0' : 'dark:border-dark-600'}`}>
